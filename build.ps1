...
# 0. 计算确定性 ProductCode（从版本号）
$ProductCode = & "$Root\QZXFrp.Setup\GenerateProductCode.ps1" `
    -UpgradeCodeGuid "5B7B2E3F-D9A3-4C12-8F6E-AB45C78D90E1" `
    -Version $Ver
Write-Host "ProductCode: $ProductCode (Version=$Ver)" -ForegroundColor Cyan
...
dotnet build "$Root\QZXFrp.Setup\QZXFrp.Setup.wixproj" -c Release -p:ProductCode=$ProductCode -p:Version=$Ver
